...
runLayout ddir mfocus0 layout = (fmap.fmap) snd $ runLayoutL ddir mfocus0 $ layout
...
runLayoutL ddir mfocus0 (Layout child) = \focusReqIx -> mdo
  dw <- displayWidth
  dh <- displayHeight
  ...
  let queries = flip appEndo [] <$> queriesEndo
      ...
      adjustFocus fm (Left (Just ix)) = do
        ...
        return (ix, x)
      adjustFocus _ (Left Nothing) = Nothing
...
tile
  ...
  -> LayoutVtyWidget t m (Event t x, a) -- ^ A child widget. The 'Event' that it returns is used to request that it be focused.
  -> Layout t m a
tile config widget = tile_ config ((fmap.fmap.fmap) ((,) dummyCell) widget)

tile_
  :: forall t m a x. (Reflex t, MonadFix m, MonadNodeId m)
  => TileConfig t -- ^ The tile_'s configuration
  -> LayoutVtyWidget t m (Event t x, (LayoutReturnData t, a)) -- ^ A child widget. The 'Event' that it returns is used to request that it be focused.
  -> Layout t m a
tile_ (TileConfig con focusable) child = mdo
  ...
  Layout $ tellDyn $ ffor3 con focusable nKiddosDyn $ \c f n -> Endo ((nodeId, (f, c), _layoutReturnData_tree (fst b), n):)
  seg <- Layout $ asks $
  ...
  let nKiddosDyn = _layoutReturnData_children $ fst b
  ...
  (focusReq, b) <- Layout $ lift $ lift $ lift $
    pane reg (demuxed focussed $ Just nodeId) $ child focusChildEv
  Layout $ tellEvent $
    gate (fmap  (>0) (current nKiddosDyn))
    . fmap (First . swap)
    $ attachPromptlyDyn (fmap (fromMaybe 0) $ _layoutReturnData_focus (fst b)) (nodeId <$ focusReq)
  return $ snd b
...
fixed
  ...
  -> VtyWidget t m a
  -> Layout t m a
fixed sz = fixed_ sz . const . fmap ((,) dummyCell)
...
stretch
  ...
  => VtyWidget t m a
  -> Layout t m a
stretch = stretch_ . const . fmap ((,) dummyCell)
...
dummyCell :: Reflex t => LayoutReturnData t
dummyCell = LayoutReturnData
...
layoutFocusEvFromNavigation
  ...
  -> LayoutReturnData t
  -> Event t (Maybe Int)
layoutFocusEvFromNavigation navEv unfocusEv lrd = r where
  fmapfn (nKiddos, (mcur, shift)) = maybe (Just 0) (\cur -> Just $ (shift + cur) `mod` nKiddos) mcur
  navEv' = attach (current $ _layoutReturnData_children lrd) $ attach (current $ _layoutReturnData_focus lrd) navEv
...
beginLayoutL child = mdo
  ...
  tabEv <- tabNavigation
  let
    focusChildEv = layoutFocusEvFromNavigation tabEv never $ fst lrd
  lrd <- child focusChildEv
  return lrd
...
beginLayout
  ...
  => LayoutVtyWidget t m (LayoutReturnData t, a)
  -> VtyWidget t m a
beginLayout = fmap snd . beginLayoutL
...
dynLayoutTreeInDynRegion region dlt = ffor3 (_dynRegion_left region) (_dynRegion_top region) dlt $ \x y lt -> fmap (offsetRegion (x,y)) lt
...
data LayoutReturnData t = LayoutReturnData
...
type LayoutVtyWidget t m a = Event t (Maybe Int) -> VtyWidget t m a

-- class (Monad m, Functor (l t m)) => IsLayoutVtyWidget l t m where
...
-- 
-- instance Monad m => IsLayoutVtyWidget VtyWidget t m where
--   runIsLayoutVtyWidget w _ = w
-- 
-- instance Monad m => IsLayoutVtyWidget LayoutVtyWidget t m where
