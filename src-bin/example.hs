...
{-# OPTIONS_GHC -threaded -Wall -Werror #-}
...
main = mainWidget $ do
  ...
  let buttons = beginLayout $ col $ do
        ...
        d <- fixed 5 $ textButtonStatic def "Layout/Focus"
        return $ leftmost
          [ Left Example_Todo <$ a
          , Left Example_TextEditor <$ b
          , Left Example_ScrollableTextDisplay <$ c
          , Left Example_Layout <$ d
          ]
      escapable w = do
        void w
        i <- input
  ...
  rec out <- networkHold buttons $ ffor (switch (current out)) $ \case
        ...
        Left Example_Layout -> escapable easyExample
        Right () -> buttons
  return $ fforMaybe inp $ \case
    V.EvKey (V.KChar 'c') [V.MCtrl] -> Just ()
    _ -> Nothing
...
todos todos0 newTodo = do
  ...
  rec tabNav <- tabNavigation
      ...
      (lrd, widget) <- flip ($) focusEv $ runLayoutL (pure Orientation_Column) (Just 0) $
        listHoldWithKey todosMap0 updates $ \k t -> tile tileCfg $ \_ -> do
      ...
      let delete = ffor todoDelete $ \k -> Map.singleton k Nothing
          ...
          todoDelete = switch . current $
            leftmost .  Map.elems . Map.mapWithKey (\k -> (k <$) . _todoOutput_delete) <$> widget
          todosMap = joinDynThroughMap $ fmap _todoOutput_todo <$> widget
